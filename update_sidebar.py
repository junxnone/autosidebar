...
def dump_kg_json(df, reponame, rname):
    ...
    rootnode = {"id": reponame,"group":0, "title": rname,"url":url, "visible": True, "expand": True, "childLinks": []}
    ...
    for i in range(1,10):
        ...
        if i in df.columns:
            ...
            for index,row in subdf.iterrows():
                ...
                node = {"id": nodeid,"group":i, "title": row['title'], "url":nodeurl, "visible": True, "expand": True, "childLinks": []}
                kg["nodes"].append(node)
            if(i-1 == 0):
                node = {"id": nodeid,"group":i,"url":nodeurl}
                for tnode in nodelist[str(i)]:
                    link = {"source": nodelist["0"][0], "target": tnode, "value": i, "visible": True, "expand": True}
                ...
                if(str(i-1) in nodelist):
                    for snode in nodelist[str(i-1)]:
                        for tnode in nodelist[str(i)]:
                            if snode in tnode:
                                link = {"source": snode, "target": tnode, "value": i, "visible": True, "expand": True}
                                kg["links"].append(link)
